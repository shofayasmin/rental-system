# Synthetic Excel Import Spec (v1.2)
...
- `queued` tidak boleh muncul (status ini sudah dihapus dari schema dan dimap ke `pending_review`).
...
- unique pair `(tenant_id, agent_id)` di conversations tetap valid
...
1. Backup jika perlu.
2. Reset domain data (replace mode).
3. Pastikan `admin@test.com` di-create via upsert.
4. Jalankan importer sesuai urutan sheet di atas.
...
|---|---|---|---|
| v1.1 | 2026-05-23 | First practical import spec aligned with v1.1 schema snapshot | Codex + Shofa |
| v1.2 | 2026-05-26 | Remove stale `rental_requests.message`, enforce no-queued note, update full import order with contracts/extensions before transactions, add conversation uniqueness check | Codex + Shofa |
