...
class UpdatePropertyRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return auth()->check()
            ...
            && $property
            && (int) $property->agent_id === (int) auth()->id();
    }
    ...
    private function validateStatusSpecificRules($validator): void
    {
        ...
        $newPhotos = (array) $this->file('photos', []);
        $existingPhotoCount = $property?->photos()->count() ?? 0;
        ...
        if ($hasOngoingLease) {
            $validator->errors()->add('status', 'Property still has an active lease and cannot be set to To Let.');
        }
